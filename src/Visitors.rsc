...
import lang::java::jdt::JDTRefactoring;

public loc rascalProjectLoc = |project://rascal|;
...
@doc{Given an interface, return all interfaces that directly or indirectly extend it.}
public set[Entity] findExtenders(Resource r, Entity interface) {
	set[Entity] extenders = { interface };
	solve (extenders) { 
		extenders = extenders + { getInterfaceExtenders(r, e) | e <- extenders }; 
	}
	return extenders;
}

@doc{Given a set of interfaces, return all classes that directly or indirectly implement one or more of the interfaces in the set.}
public set[Entity] findImplementers(Resource r, set[Entity] interfaces) {
	set[Entity] implementers = { getInterfaceImplementers(r, i) | i <- interfaces };
	solve (implementers) { 
		implementers = implementers + { getClassExtenders(r, i) | i <- implementers }; 
	}
	return implementers;
}

@doc{Build a list of the classes that we don't want to transform.}
public set[Entity] classesToKeep(Resource r, set[Entity] implementers) {
	return { i | i <- implementers, 
		entity([_*,class("BoxEvaluator"),_*]) := i ||
		entity([_*,class("StringTemplateConverter"),_*]) := i || 
		entity([_*,class("TestEvaluator"),_*]) := i  || 
		entity([_*,package("debug"),_*]) := i || 
		entity([_*,class("IUPTRAstToSymbolConstructor"),_*]) := i || 
		entity([_*,class("DeclarationCollector"),_*]) := i };
}

@doc{Extract the resource information from the Rascal project.}
public Resource extractRascal() {
	return extractProject(rascalProjectLoc);
}
...
public bool cleanUpImplementers() {
	...
	set[Entity] extenders = findExtenders(rascal, astVisitor);

	// Starting with interfaces, get all reachable implementing classes (based on implements and extends relations)
	set[Entity] implementers = findImplementers(rascal, extenders);

	// Remove classes that should continue to use the Visitor pattern from the
	// set of implementers that we want to transform.
	implementers = implementers - classesToKeep(rascal, implementers);
...
public list[&T] filterList(list[&T] l, bool(&T) f) { 
	return [ x | x <- l, f(x)]; 
}
...
public str makeEvalMethodName(Entity e) {
	s = collapseStrList([ getClassName(x) | entity(l) := e, x <- filterList(l,bool(Id i) { return (class(_) := i || class(_,_) := i); }) ]);
